...
class BytecodeClassGenerator implements IClassGenerator {

    @Override
    Class<?> generateRule(Rule rule) {
        return new InternalBytecodeClassGenerator()
                .generateClassForRule(rule, this.getClass().getClassLoader())
    }

    private class InternalBytecodeClassGenerator {
        ...
        def private genClassSkeleton(String rule_name) {
            // generate class name
        ...
        // TODO: refactoring
        def private genClassFields(Rule rule, DynamicType.Builder<?> dynamicType) {
        ...
        def private genMethods(Rule rule, DynamicType.Builder<?> dynamicType) {
        ...
        def private genRule(DynamicType.Unloaded<?> dynamicType, ClassLoader classLoader) {
